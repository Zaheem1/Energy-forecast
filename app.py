...
# ══════════════════════════════════════════════════════════════════════════
    ...
    with d1:
        gdp_eu  = st.slider("GDP per energy unit (PPP $/kg oil eq.)", 3.0, 15.0, 8.5, 0.1)
        energy_u= st.slider("Energy use (kg oil eq./capita)",        300.0, 800.0, 490.0, 5.0)
    with d2:
        fossil_e= st.slider("Fossil fuel consumption (%) ", 40.0, 90.0, 62.0, 0.5)
        elec_c  = st.slider("Electric power consumption (kWh/capita)", 200.0, 700.0, 470.0, 5.0)
    with d3:
        coal_e  = st.slider("Coal electricity prod. (%) ", 0.0, 40.0, 20.0, 0.5)
        renew_c = st.slider("Renewable energy consumption (%)", 15.0, 55.0, 42.0, 0.5)
        imports_e= st.slider("Net energy imports (%) ", 0.0, 35.0, 15.0, 0.5)
        tnd_loss= st.slider("T&D losses (% of output)", 10.0, 30.0, 18.0, 0.5)
    ...
    with g1:
        ...
        st.caption(f"Historical range: {hist_min_r:.1f}% – {hist_max_r:.1f}%  |  Prediction: {pred_r:.2f}%")
    with g2:
        pct_e = (pred_e - hist_min_e) / (hist_max_e - hist_min_e) * 100
        st.progress(min(max(int(pct_e), 0), 100))
        st.caption(f"Historical range: {hist_min_e:.2f} – {hist_max_e:.2f}  |  Prediction: {pred_e:.3f}")
